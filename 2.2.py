...
# Load the dataset
dataset_dir = 'horse-or-human'  # Replace with actual path
...
for images, labels in train_data.take(1):  # Take one batch from the test set
    ...
    for i in range(9):  
        ...
        plt.title(int(labels[i].numpy()))  # Convert to integer for display
        plt.axis("off")
    plt.show()
...
y_pred = np.round(np.concatenate(model.predict(val_data)).flatten())  # Flatten predictions
