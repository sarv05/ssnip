...
def canny_edge_detection(image):
    ...
    strong_edges, weak_edges = double_threshold(non_max_suppressed)
...
def non_maximum_suppression(magnitude, angle):
    ...
    for i in range(1, rows-1):
        for j in range(1, cols-1):
            # Suppression based on direction
            q, r = 255, 255
...
# Example usage
image = cv2.imread('image.jpg', cv2.IMREAD_GRAYSCALE)  # Read the image as grayscale
